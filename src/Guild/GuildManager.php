...
class GuildManager{
    ...
    public function promoteofficer(Player $player){
        $playername = $player->getName();
        if($this->plugin->getPlayerManager()->getPlayerRank($player) !== "LEADER") {
    ...
    public function kickmember(Player $kicker, string $player){
        ...
        if($this->plugin->getPlayerManager()->getPlayerRank($kicker) == "LEADER"){
            ...
            $this->plugin->getDatabase()->query("UPDATE guildinfo SET memberguild=$member - 1 WHERE guildname='$guildname[0]'");
